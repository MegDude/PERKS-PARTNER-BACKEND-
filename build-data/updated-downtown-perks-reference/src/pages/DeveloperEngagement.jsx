...
export default function DeveloperEngagement() {
  ...
  const totalResidents = tenants.length || 487;
  const enrolledResidents = tenants.filter(t => t.perks_enrolled).length || Math.round(487 * 0.68);
  const overallEngagement = totalResidents > 0 ? Math.round((enrolledResidents / totalResidents) * 100) : 68;

  const buildingStats = buildings.length > 0 ? buildings.map(building => {
    const buildingTenants = tenants.filter(t => t.flat_id?.includes(building.id));
  ...
  return (
    <div className="min-h-screen bg-bgMain p-6">
      <div className="max-w-7xl mx-auto">
        {/* Header */}
        <div className="mb-8">
          <h1 className="text-3xl font-bold text-navy mb-2">Portfolio Engagement</h1>
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-8">
          <Card className="border-gold/20 bg-gradient-to-br from-white to-gold/5">
            <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
              <CardTitle className="text-sm font-medium text-navy">Buildings</CardTitle>
              <Building2 className="h-4 w-4 text-gold" />
            </CardHeader>
            <CardContent>
              <div className="text-2xl font-bold text-navy">{buildings.length}</div>
              <p className="text-xs text-slate-500">Properties managed</p>
            </CardContent>
          </Card>

          <Card className="border-gold/20 bg-gradient-to-br from-white to-gold/5">
            <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
              <CardTitle className="text-sm font-medium text-navy">Total Residents</CardTitle>
              <Users className="h-4 w-4 text-gold" />
            </CardHeader>
            <CardContent>
              <div className="text-2xl font-bold text-navy">{totalResidents}</div>
              <p className="text-xs text-slate-500">Across portfolio</p>
            </CardContent>
          </Card>

          <Card className="border-gold/20 bg-gradient-to-br from-white to-gold/5">
            <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
              <CardTitle className="text-sm font-medium text-navy">Perks Adoption</CardTitle>
              <TrendingUp className="h-4 w-4 text-gold" />
            </CardHeader>
            <CardContent>
              <div className="text-2xl font-bold text-navy">{overallEngagement}%</div>
              <p className="text-xs text-slate-500">{enrolledResidents} enrolled</p>
            </CardContent>
          </Card>

          <Card className="border-gold/20 bg-gradient-to-br from-white to-gold/5">
            <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
              <CardTitle className="text-sm font-medium text-navy">Messages Sent</CardTitle>
              <MessageSquare className="h-4 w-4 text-gold" />
            </CardHeader>
            <CardContent>
              <div className="text-2xl font-bold text-navy">{broadcasts.filter(b => b.delivery_status === 'sent').length}</div>
        ...
        {tenants.length === 0 && (
          <Card className="mb-6 bg-gold/10 border-gold/30">
            <CardContent className="pt-6">
              <p className="text-sm text-navy">💡 <strong>Demo Mode:</strong> Showing sample data. Real metrics will appear here once you add buildings and residents to the platform.</p>
            </CardContent>
          </Card>
        )}
        ...
        <Card className="mb-6">
          <CardHeader>
            <CardTitle className="text-navy">Adoption Trend</CardTitle>
          ...
          <CardContent>
            <ResponsiveContainer width="100%" height={300}>
              <LineChart data={portfolioTrend}>
                ...
                <Tooltip />
                <Legend />
                <Line type="monotone" dataKey="adoption" stroke="#CFAF5A" name="Adoption Rate (%)" strokeWidth={2} />
              </LineChart>
            </ResponsiveContainer>
          </CardContent>
        </Card>
        ...
        <Card className="mb-6">
          <CardHeader>
            <CardTitle className="text-navy">Building Performance</CardTitle>
          ...
          <CardContent>
            <ResponsiveContainer width="100%" height={400}>
              <BarChart data={buildingStats}>
                ...
                <Bar dataKey="enrolled" fill="#CFAF5A" name="Perks Enrolled" />
              </BarChart>
            </ResponsiveContainer>
          </CardContent>
        </Card>
        ...
        <Card>
          <CardHeader>
            <CardTitle className="text-navy">Building Breakdown</CardTitle>
          ...
          <CardContent>
            <div className="space-y-4">
              {buildingStats.map((stat, idx) => {
                ...
                return (
                  <div key={idx} className={`flex items-center justify-between p-4 border rounded-lg transition-all ${
                    isAboveAverage ? 'border-gold/30 bg-gold/5' : 'border-slate-200'
                  }`}>
                    <div>
                      <h3 className="font-semibold text-navy">{stat.name}</h3>
                      <p className="text-sm text-textSecondary">{stat.residents} residents • {stat.enrolled} enrolled</p>
                    </div>
                    <div className="text-right">
                      <div className="text-2xl font-bold text-navy">{stat.engagement}%</div>
